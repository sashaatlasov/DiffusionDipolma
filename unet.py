...
class UnetModel(nn.Module):
    def __init__(self, in_channels: int, out_channels: int, hidden_size: int = 256):
        ...
        self.point_embedding = ConditionEmbedding(2, 2 * hidden_size)
        self.momentum_embedding = ConditionEmbedding(3, 2 * hidden_size)
    ...
    def forward(self, x: torch.Tensor, m: torch.Tensor, 
        ...
        memb = self.momentum_embedding(m)[:, :, None, None]
        pemb = self.point_embedding(p)[:, :, None, None]

        thro = self.up0(thro + temb + memb + pemb)

        up1 = self.up1(thro, down3) + temb + memb + pemb
